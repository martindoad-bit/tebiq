...
export const FACT_ANCHORS: ReadonlyArray<FactAnchor> = Object.freeze([
  ...
  },
  {
    ...
    // DOMAIN 2026-05-17 Q7 fix: 不再硬編码"直近5年"作为通用窓口。
    // 官方審査範圍按申請ルート区分（就労資格者≠日配・永配≠HSP）。
    // 提交範囲・審査窓口の具体年数は申請ルート確認、納税・年金・健保の継続加入は共通要件として残す。
    mustConsider:
      '技人国から永住申請には原則10年以上の在留（うち就労5年以上）が必要。' +
      '納税・年金・健康保険の継続加入と直近1年の出国日数が審査対象となる。' +
      '提出書類の対象期間（住民税・国税・年金・健保の各証明）は申請ルート（就労資格者 / 日配・永配 / 高度人材）によって異なるため、最新のISA案内で個別確認が必要。',
    mustNotSay: '申請できます / 許可されます / 10年いれば大丈夫 / 条件を満たしています / 直近5年分すべて必要',
    suggestedNextQuestion: '現在の在留年数・健康保険と年金の継続加入状況・直近の出国期間・申請予定ルート（就労資格者か配偶者ルートか）を教えてください。',
    humanConfirmHint: '納税漏れ・年金未払い・出国超過がある場合、または提出書類の対象期間に不明点がある場合は行政書士への確認を推奨。',
  },
  {
    anchorId: 'pension_pr',
    triggerKeywords: [
    ...
    // DOMAIN 2026-05-17 Q7 fix: "直近5年"窓口は申請ルート別。
    // 一律で"5年分必要"と断定しない。期限後納付は記録に残り消極評価対象となり得る、
    // 未納発生時点・免除/猶予の有無・補填の有無を区別して確認する。
    mustConsider:
      '永住審査では年金保険料の納付実績と継続加入状況が確認される。' +
      '提出書類の対象期間（直近何年分）は申請ルートにより異なるため、最新のISA案内で確認が必要。' +
      '期限後の追納でも納付記録には未納期間として残り、消極的に評価される可能性がある。' +
      '免除・猶予の承認を受けた期間は未納とは扱いが異なる場合があるが、個別状況による。',
    mustNotSay: '未払いがあっても大丈夫 / 今払えば問題なし / 影響しません / 必ず許可されます / 直近5年分すべて必要',
    suggestedNextQuestion: '年金の未納が発生した時期・補填済みか・免除/猶予の申請があったかを教えてください。',
  ...
  },
  {
    ...
    // DOMAIN 2026-05-17 Q7 fix: 税务審査窓口も申請ルート別。
    // 住民税は5年、国税（その3）は3年など書類ごと年数が異なる。
    mustConsider:
      '如果用户明确说不是问永住/PR，应只回答当前税务缴纳、分割納付、滞纳记录状态，不主动扩展到永住审查。' +
      '永住申請では住民税・所得税の納税証明が必要。' +
      '対象期間は書類ごとに異なる（住民税の課税/納税証明書、国税納税証明書その3など）。' +
      '具体的な年数は申請ルートと最新のISA案内で確認が必要。' +
      '未納・滞納や確定申告漏れは許可に影響する可能性がある。',
    mustNotSay: '少し遅れても大丈夫 / 今払えば問題なし / 影響しません / 許可されます / ユーザーが明示的に永住を除外しているのに永住リスクへ広げる',
    suggestedNextQuestion: '住民税や所得税の未納・滞納の経験、確定申告漏れの有無を教えてください。',
