...
function cannotDetermineAnswer(_questionText: string): AnswerResult {
  return {
    ...
    related_links: [
      { title: '拍照识别文书', href: '/photo' },
      { title: '咨询记录', href: '/me/consultations' },
    ],
    sources: [],
    query_id: null,
    answer_id: null,
...
function ruleBasedAnswerForIntent(questionText: string, intent: AnswerIntent): AnswerResult | null {
  ...
  if (isCompanyDormantPensionIntent(intent)) {
    return {
      ...
      related_links: [{ title: '咨询记录', href: '/me/consultations' }],
  ...
  if (intent.domain === 'employment' && intent.current_status === '特定技能' && /雇主|雇用主|会社|公司|転職|换/.test(questionText)) {
    return {
      ...
      related_links: [{ title: '提问', href: '/ai-consultation' }],
...
function isFamilyWorkPermissionIntent(intent: AnswerIntent, questionText: string): boolean {
  // Family stay → work visa (在留資格変更) is a different track. Don't
  ...
  // Drop bare 工作 from work-context regex — `工作签` should be handled
  // by the explicit short-circuit above; without dropping bare 工作 the
  // regex would match `转工作签` again.
