...
export const QUESTIONS: Record<string, Question> = {
  '1': {
    ...
    learnMore:
      '换公司是常见情况，本身完全合法。关键是入管局要求换公司后 14 天内做一个申报。如果你是自营业、经营管理签证或长期留学结束后的状态，请选「不适用」。',
    options: [
      { label: '换过', next: '2' },
      { label: '没换过', next: '4' },
  ...
  '2': {
    ...
    why: '14 天内申报是法律规定的义务',
    learnMore:
      '这个通知在日语里叫「契約機関に関する届出」。如果你不知道自己有没有提交过，选「没有」。可以登录入管局的在线系统（e-Notification）查询，或者带在留卡去最近的入管局窗口问。',
    options: [
  ...
  '3': {
    ...
    learnMore:
      '空窗期超过 3 个月不必然导致不许可，但需要提供合理的说明（求职记录、进修证明等）。如果你是主动跳槽，通常 1-2 个月内找到新工作没问题。',
    options: [
      { label: '没有超过', next: '4' },
      {
        label: '超过了',
        ...
        selfFix: false,
        fixHint:
          '在留资格「技人国」以稳定就业为前提，无业超过 3 个月入管会怀疑你已不再符合资格条件。请准备一份从离职到入职的详细经历说明（含面试记录、求职活动），由专家判断要不要先用「特定活动」签证过渡。',
      },
      { label: '不适用', next: '4' },
    ],
  },
  '4': {
    ...
    why: '欠税是续签被拒的高频原因',
    learnMore:
      '住民税是住在日本就必须缴纳的地方税。如果你是正社员，通常公司每月从工资里自动扣缴（叫「特别征收」），这种情况一般不会欠缴。如果不确定，可以去住所所在区役所的税務課窗口查询，或者打住民票证明时顺便问。',
    options: [
      { label: '全部缴清', next: '6' },
      {
        label: '有欠缴',
        ...
        selfFix: false,
        fixHint:
          '住民税没缴是被拒签最常见的硬伤。先去市役所开「纳税证明书」和「课税证明书」看现状，欠多少补多少，留好收据。补缴时间晚一点没关系，重点是申请前必须结清。如果情况复杂，建议让专家帮你写一份「诚实改正」说明附在材料里。',
      },
      {
        label: '不确定',
        next: '6',
        ...
        selfFix: true,
        fixHint:
          '不确定就当作存在待确认事项。带在留卡去住所所在区役所税務課窗口查询，5 分钟即可出具納税証明書/課税証明書。结清后留好收据。',
      },
    ],
  },
  '6': {
    ...
    why: '社保连续性是续签审查项目',
    learnMore:
      '如果你是正社员，公司会自动帮你加入社会保険（健康保险+厚生年金），这种情况一般不会断。但换工作空窗期间，你需要自己去市役所加入国民健康保险，否则这段时间就是「无保险」状态。如果你中间有换工作的空窗期，而且没去办理国保，就算「断过」。',
    options: [
      { label: '没有断过', next: '7' },
      {
        label: '有断过',
        ...
        selfFix: false,
        fixHint:
          '从 2020 年起入管已经把社保参保情况纳入审查重点。先去年金事务所或区役所补办手续，再准备一份说明书解释断保原因——换工作的过渡期是常见且可接受的理由，建议由专家帮你写。',
      },
      {
        label: '不确定',
        next: '7',
  ...
  '7': {
    ...
    learnMore:
      '审查期间（通常 1-3 个月）是合法的等待期，叫做「特例期间」，不算超期居住。但如果到期日已过还没递交申请，就属于违法居住，后果严重。',
    options: [
      { label: '到期前递交', next: '8' },
      {
        label: '超期才递交',
        ...
        selfFix: false,
        fixHint:
          '在留期限届满后才提交属于不法残留，哪怕只超 1 天也会留记录。这种情况待确认事项很高，请不要自行递签。请尽快和专家面谈，评估特定活动签证或重新出入境的可行性，每多拖一天处理空间都会变小。',
      },
  ...
  '8': {
    ...
    why: '违规记录会影响续签审查',
    learnMore:
      '严重违规包括：任何刑事违规（即使只是罚款）、酒驾、无证驾驶、重大交通事故肇事逃逸等。普通超速、一般交通违反影响较小。不确定是否严重可以联系专家评估。',
    options: [
      { label: '没有', next: '10' },
      { label: '有', next: '9' },
    ],
  ...
  '9': {
    ...
    why: '不同等级违规对续签影响不同',
    learnMore:
      '入管会调取你的违规记录。轻微违规（普通超速、违规停车）影响较小；刑事违规（即使只是罚金）和严重交通违规（酒驾、无证、肇事逃逸）会直接影响审查结果。',
    options: [
      {
        ...
        selfFix: true,
        fixHint:
          '轻微交通违规一般影响有限，但隐瞒比主动说明更不利。入管会调取你的违规记录。在申请书上主动说明并附违反通告书副本即可，不需要专家介入。',
      },
      {
        ...
        selfFix: false,
        fixHint:
          '酒驾、无证驾驶、刑事案件入管极其敏感，可能直接导致拒签。这种情况必须由专家介入，我们会判断如何附上法院文书或处分决定书，并设计陈述方式将影响降到最低。',
      },
    ],
  },
  '10': {
    ...
    why: '雇主状况影响续签审查',
    learnMore:
      '如果你不确定公司的经营状况，可以观察：工资是否按时发放、是否有大规模裁员、客户业务是否正常。不确定的话可以先问同事或 HR。\n\n2026 年 4 月 15 日新规（已生效）：如果你的雇主属于类别 3 或 4（非上市中小企业），且工作以日语对人业务为主（翻译、营业、前台接待等），续签时需额外提交：①所属机构代表者申报书 ②JLPT N2 以上等语言能力证明。IT 工程师等技术岗位是否需要目前尚不明确，建议提前向公司人事确认。',
    options: [
      { label: '正常运营', next: '12' },
      { label: '不正常', next: '11' },
      {
  ...
  '11': {
    id: '11',
    text: '公司具体是什么情况？',
    why: '公司问题的严重程度决定递交前确认顺序',
    learnMore:
      '公司规模缩小但仍在正常经营通常影响有限，需要提供最新决算报告书证明公司财务状况。公司已经停业或破产的情况下，原则上技人国续签会受到严重影响，需要立刻咨询专家。',
    options: [
      {
        label: '规模缩小但还在运营',
        next: '12',
        ...
        selfFix: false,
        fixHint:
          '公司缩小本身不致命，但要主动证明你的岗位仍然稳定存在。建议由专家帮你整理最新的决算书、雇用合同更新版，必要时附一份公司说明书解释经营情况，避免入管单方面怀疑。',
      },
      {
        label: '已经停业或破产',
        next: '12',
  ...
  '12': {
    ...
    why: '年收是续签审查的重要参考指标',
    learnMore:
      '这里的年收是税前总收入，包括基本工资+奖金+各种津贴。不确定的话可以看最近一份源泉徴収票（每年 1 月公司发的那张纸）上面的「支払金額」。技人国没有硬性最低年收要求，但入管局要求「不低于从事同等工作的日本人」。250 万日元以下需要准备额外说明材料。',
    options: [
  ...
  '13': {
    ...
    why: '护照必须在续签申请期间保持有效',
    learnMore:
      '建议：如果护照 1 年内到期，先去中国驻日大使馆/总领事馆续签护照（约需 1-2 个月），再办在留续签。两者可以同步进行但要注意时间安排。',
    options: [
      { label: '6 个月以上', next: '14' },
      {
        label: '不足 6 个月',
        next: '14',
        severity: 'yellow',
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '护照已过期或即将过期是待确认情况。请立即联系中国驻日大使馆/总领馆紧急办理护照换发，并向专家咨询是否需要提前递交「再入国不要」声明等补救材料。',
      },
    ],
  },

  // === Q14-18：第 6 天专业人士反馈新增的 5 个维度（犯罪 / 出入国 / 課税 / 住民票 / 雇用） ===
  '14': {
    id: '14',
    text: '除了交通违规，你过去 5 年内有没有罚金、停止令、軽犯罪法処分等任何处分？',
    why: '入管会调取所有处分记录（警察 + 行政），不限于交通',
    learnMore:
      '常被忽视的范围：軽犯罪法処分、商店扒窃、迷惑防止条例違反、行政罚金（垃圾分类罚金不算）、不法滞在记录、入管警告、ハローワーク给付资格停止处分等。即使最终不起诉、罚金缴清，也会留在数据库中。如不确定可申请「犯罪歴証明書」查询。',
    options: [
      { label: '完全没有', next: '15' },
      {
        label: '有过罚金或軽犯罪法処分',
  ...
  '15': {
    ...
    why: '长期离日会影响入管对「居住稳定性」的判断',
    learnMore:
      '入管通过出入国記録（出入国記録）核查实际在日时间。技人国期间长期在海外（如远程办公）会影响续签和未来永住申请的「在日継続性」要件。出入国記録可在最近的入国管理局窗口当场打印。',
    options: [
      { label: '不到 3 个月', next: '16' },
      {
        label: '3 - 6 个月',
        next: '16',
        severity: 'yellow',
        ...
        selfFix: true,
        fixHint:
          '这个区间不致命但需要主动说明。续签时附 1 份「在日勤務状况书」+ 出入国記録，证明工作主要在日本完成（即使部分海外出差）。',
      },
      {
        label: '6 个月以上 - 1 年',
        next: '16',
        severity: 'yellow',
        triggerLabel: '过去 3 年累计离日 6-12 个月',
        selfFix: false,
        fixHint:
          '入管会质疑「为什么持有日本签证却长期不在日」。准备：海外原因详细说明（出差/家人/学习等）、雇主出具的「海外赴任理由书」、海外期间的薪资仍由日本公司支付的证明。建议专家介入。',
      },
      {
        label: '超过 1 年',
        next: '16',
        severity: 'red',
        triggerLabel: '过去 3 年累计离日超过 1 年',
        selfFix: false,
        fixHint:
          '待确认。入管可能直接认定不再符合「在日活动」要件。请尽快和专家面谈，评估是否需要先取得「再入国許可」延长 / 切换签证 / 接受短期在留期间。',
      },
    ],
  },
  '16': {
  ...
  '18': {
    id: '18',
    text: '你目前的雇用契約是哪种形态？',
    why: '雇用契約形态直接影响在留期间长度判定',
    learnMore:
      '入管对「就業稳定性」的判断主要看契約形態。无期限正社員 → 高分；有期限契約社員 → 中等；業務委託 / フリーランス → 容易被质疑「实际是事業主」需走経営・管理签；派遣社員 → 受 2026 年 3 月 9 日新规严格审查。試用期内续签可能拿到较短在留期间。',
    options: [
      { label: '无期限の正社員', next: null },
      {
        label: '有期限契約社員（已转正前提）',
        next: null,
        severity: 'yellow',
        triggerLabel: '有期限契約社員（非派遣）',
        selfFix: false,
        fixHint:
          '续签可能拿到较短在留期间（1 年居多）。准备：契約更新通知书、雇主对未来更新的承诺函（如有）、最近 6 个月工资证明。',
      },
      {
        ...
        selfFix: false,
        fixHint:
          '建议等試用期結束、转为正式员工后再递签；如已临近在留期限，附试用期合同 + 雇主出具的「採用予定書」表明会转正。',
      },
      {
        label: '派遣社員',
        next: null,
        severity: 'red',
        triggerLabel: '派遣形态雇用（2026/3 新规严格化）',
        selfFix: false,
        fixHint:
          '2026 年 3 月 9 日新规对派遣形态大幅严格化。需准备：派遣元 / 派遣先双方誓约书、派遣先業務内容説明資料、最近 6 个月工资证明。强烈建议专家介入。',
      },
      {
        label: '業務委託 / フリーランス',
        next: null,
        severity: 'red',
        triggerLabel: '業務委託形态（非雇用关系）',
        selfFix: false,
        fixHint:
          '業務委託在技人国下需要递交前确认，可能被认定为「実际は経営者」需走経営・管理。请尽快和专家面谈评估是否切换签证类别。',
      },
      {
        label: '目前無職 / 离职中',
        next: null,
        ...
        selfFix: false,
        fixHint:
          '无在职状态时不应直接续签技人国。请评估「特定活动」过渡签 / 尽快入职新雇主再续 / 切换签证类别。',
      },
    ],
  },
}
