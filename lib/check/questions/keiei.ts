...
const Q: Record<string, QuizQuestion> = {
  ...
  q11: {
    ...
    learnMore:
      '常被忽视的范围：軽犯罪法処分、商店扒窃、迷惑防止条例違反、行政罚金、不法滞在记录、入管警告、ハローワーク给付资格停止处分等。即使最终不起诉、罚金缴清，也会留在数据库中。如不确定可申请「犯罪歴証明書」查询。[待书士审核]',
    options: [
      { label: '完全没有', next: 'q12' },
      {
        label: '有过罚金或軽犯罪法処分',
  ...
  q12: {
    ...
    learnMore:
      '入管通过出入国記録核查实际在日时间。経営管理签证下，申请人长期在海外难以证明「亲自経営」，可能被认定不再符合资格要件。出入国記録可在最近的入国管理局窗口当场打印。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '入管会质疑「为什么持有経営管理签证却长期不在日経営」。建议书士介入：准备在日経営的实质证据（决策记录、银行账户使用、与员工 / 税理士的会议记录等）。',
      },
    ],
  },
  q13: {
  ...
  q15: {
    ...
    learnMore:
      '与就劳系签证不同，経営管理审查重点是公司本身的経営健康度。営業赤字 / 客户单一 / 现金流紧张都会让入管质疑「事業の継続性」。决算書、销售合同、银行流水都是核心证据。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '事業継続性是経営管理签证的根基。请尽快和书士+税理士面谈：评估是否需要追加出资 / 増加客户多様性 / 用個人储蓄証明補足，必要时考虑切换签证类别。',
      },
      {
        label: '已停业 / 准备解散',
        next: null,
        ...
        selfFix: false,
        fixHint:
          '無事業状态下原则上无法续签経営管理。请评估「特定活动」过渡 / 重新设立事業 / 切换签证类别，立即和书士面谈。',
      },
    ],
  },
}
