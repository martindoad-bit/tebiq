...
const Q: Record<string, QuizQuestion> = {
  ...
  q8: {
    ...
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '入管视角下这是高风险信号。不要自行递签。请尽快和书士面谈，评估是否需要先取得「在留特別許可」。',
      },
      { label: '不确定', next: 'q9' },
    ],
  },
  q9: {
    ...
    learnMore:
      '入管通过出入国記録核查实际在日时间。定住者签证下，长期不在日会让入管质疑「日本での生活基盘」。出入国記録可在最近的入国管理局窗口当场打印。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '入管会重点审查「在日生活基盘」是否实质存在。建议书士介入，准备：出境理由详细说明、家族与日本的联系证据、未来在日定居计划。',
      },
    ],
  },
  q10: {
    ...
    learnMore:
      '課税証明書由住所所在地市役所「税務課」发行。中途搬家、未做転入届、当年没有収入也会让某些年份开不出。可以提前去市役所窗口确认能开出哪几年。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '说明你过去若干年与日本税务系统断链，入管会重点审查。请书士介入，评估如何重建税务证明（含補正申告等）。',
      },
      { label: '不确定', next: 'q11' },
    ],
  },
  ...
  q12: {
    ...
    learnMore:
      '定住者签证可以无収入由家族支持，但若有自己的雇用関係，正社員 → 高分；契約 / 派遣 → 中等；業務委託 → 中性；無職 → 需补充家族支持証明。[待书士审核]',
    options: [
      { label: '正社員', next: null },
