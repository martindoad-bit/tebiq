...
const Q: Record<string, QuizQuestion> = {
  ...
  q10: {
    ...
    learnMore:
      '常被忽视的范围：軽犯罪法処分、商店扒窃、迷惑防止条例違反、行政罚金、不法滞在记录、入管警告等。即使最终不起诉、罚金缴清，也会留在数据库中。如不确定可申请「犯罪歴証明書」查询。[待书士审核]',
    options: [
      { label: '完全没有', next: 'q11' },
      {
        label: '有过罚金或軽犯罪法処分',
  ...
  q11: {
    ...
    why: '特定技能要求在认定雇主处实际工作，长期不在日是高风险',
    learnMore:
      '入管通过出入国記録核查实际在日时间。特定技能签证持有者长期不在日往往意味着脱离雇主、未實際从事认定业务。出入国記録可在最近的入国管理局窗口当场打印。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '入管会质疑「为什么持有特定技能签证却长期不在日」。请书士介入，准备雇主侧的説明 + 海外期间薪资仍由日本公司支付的证明。',
      },
    ],
  },
  q12: {
    ...
    learnMore:
      '課税証明書由住所所在地市役所「税務課」发行。技能実習从特定技能切换的，请确认当年的课税情况是否能开出。可以提前去市役所窗口确认。[待书士审核]',
    options: [
  ...
  q13: {
    ...
    learnMore:
      '正常情况：搬家做転入届、海外长期出差不办「海外転出届」就保留登録。脱漏发生在：办了海外転出届但回来后没及时転入、搬家后忘办転入届超 14 天等。[待书士审核]',
    options: [
      { label: '没有脱漏', next: 'q14' },
      {
  ...
  q14: {
    ...
    learnMore:
      '特定技能 1 号原则上禁止派遣（农业・渔业除外）；2 号根据業種有不同规定。業務委託 / フリーランス 不符合特定技能资格要件。[待书士审核]',
    options: [
      ...
      },
      {
        ...
        selfFix: false,
        fixHint:
          '特定技能 1 号原则禁止派遣（仅农业・渔业例外）。请尽快和书士面谈，确认是否需要更换雇主。',
      },
      {
        label: '業務委託 / フリーランス',
        next: null,
        ...
        selfFix: false,
        fixHint:
          '特定技能要求与认定雇主的直接雇用关系，業務委託不符合资格要件。可能被认定为不法就労。立即和书士面谈。',
      },
      {
        label: '目前無職 / 离职中',
        next: null,
