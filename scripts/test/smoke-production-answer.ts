...
const REDLINES: Redline[] = [
  ...
  },
  {
    ...
    reject: /(当前判断|结论|所以|因此).{0,40}(再婚|后续结婚).{0,40}(覆盖|代替|不用|无需).{0,20}(离婚届出|届出义务|离婚手续)/,
  ...
  },
  {
    ...
    reject: /(COE|签证|ビザ).{0,24}(一定可以|一定能|肯定可以|肯定能|保证可以|保证能|保証).{0,24}(入国|入境|上陸|过关)|机场.{0,20}(不会|不再).{0,20}(看|审查).{0,20}(过去|记录)|(^|[。；\n])[^。；\n]{0,12}(大多数情况下|通常).{0,12}(可以|能).{0,12}(顺利入境|入境|入国)/,
