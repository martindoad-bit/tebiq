...
export default function EvalConsoleClient() {
  ...
  const [sortBy, setSortBy] = useState<'tag' | 'class'>('tag')
  ...
  const filteredSorted = useMemo(() => {
    ...
    return list
  }, [questions, answers, classByQuestion, classFilter, scenarioFilter, statusFilter, sortBy])
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-mono">
      <header className="border-b border-slate-300 bg-white px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        {loadError && (
          <span className="text-xs text-red-600">载入错误: {loadError}</span>
        )}
      ...
      <section className="px-3 pb-2 flex flex-wrap items-center gap-2 text-xs">
        ...
        <FilterSelect
          label="排序"
          value={sortBy}
          onChange={v => setSortBy(v as 'tag' | 'class')}
          options={[
            ['tag', 'starter_tag'],
            ['class', 'sample class'],
          ]}
              ...
              {filteredSorted.map(q => {
                ...
                return (
                  <tr key={q.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-2 py-1.5 max-w-[28rem]">
