...
import { Camera, CheckSquare, ClipboardCheck, Languages } from 'lucide-react'
...
const TOOLS: Array<{
  ...
  { title: '任务清单', desc: '把手续拆成可执行步骤', Icon: CheckSquare },
]
  ...
  return (
    <div className="pt-3">
      <section className="rounded-card border border-hairline bg-surface px-4 py-4 shadow-card">
        <h1 className="text-[17px] font-medium leading-snug text-ink">你今天要处理什么？</h1>
        <p className="mt-1 text-[12px] leading-[1.65] text-ash">
          文件、文字、自查和提醒，先从最贴近的一项开始。
        </p>
      </section>
  ...
  if (!href) {
    return (
      <button
        ...
        className="min-h-[142px] rounded-card border border-hairline bg-surface px-3.5 py-3.5 text-left shadow-card transition active:translate-y-px"
      >
        {inner}
      </button>
    )
