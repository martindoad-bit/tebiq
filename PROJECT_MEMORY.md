...
Visual Polish 2 后，TEBIQ 不再使用「政府蓝 + 明橙 + 奶油米」作为主框架。新的目标是：像日本生活工具产品，而不是 generic SaaS 或儿童化 AI 插画产品。

- 调性：温暖陪伴 + 工具感 + 日式克制。页面要像可长期使用的生活文具：安静、清楚、可信，不喊口号。
- 主色：墨青 #233B37。用于标题、正文重点、主图标和深色控件；比政府蓝更成熟，也避免落入行政网站感。
- 强调色：赤金 #C49A5A。用于 CTA、进度、状态强调；饱和度低于旧橙色，不做大面积亮黄。
- 背景：纸白灰 #F6F4EF；卡片：#FFFEFA；浅强调底：#F0E7D8；边线：rgba(35, 59, 55, 0.105)。
- 辅助色：灰绿蓝 #E7EEE9、成功 #557C65、警示 #B45A4E、正文灰 #46534F、次级灰 #707A75、浅灰 #B7B1A7、chip #ECE8DF。
...
- 圆角：卡片 10px，按钮 9px，标签 / chip 7px。相机取景框、头像等功能性容器可按语义放大，但不把所有东西做成统一大圆角。
- 阴影：中性阴影，不用主色染阴影。卡片使用 `0 1px 0 rgba(30,28,22,.035), 0 9px 22px rgba(34,42,38,.052)`；raised 使用更宽但低透明度的灰绿阴影；CTA 只保留轻微赤金投影。
- 图标：优先 lucide 线性图标，小尺寸 stroke-width 1.5-1.7；不要混用硬填色插画图标和线性图标；旧 #1E3A5F / #F6B133 不再硬编码。
- 插画：优先手工 SVG 或极简几何图形，不使用 AI 生成的人物、礼物盒、清单、文件角色。插画控制在 2-3 色、留白多、线条克制，像文具说明图而不是营销插图。
