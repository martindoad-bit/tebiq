---
slug: check-management-entry-exit-record
title: "出入国记录 / 连续居住"
visa_type: management
dimension_key: entry_exit_record
dimension_version: 1
priority: normal
expiry_days: 30
estimated_read_time_minutes: 3
scenario_tags: [续签前, 出国前, 出国频繁, 想长期定居]
applies_to: ["経営・管理"]
urgency_level: low
questions:
  - id: q1
    text: "过去 1 年单次出国是否有超过 90 日？"
    type: yes_no_unknown
  - id: q2
    text: "过去 1 年累计出国是否超过 100 日？"
    type: yes_no_unknown
  - id: q3
    text: "出国时全部办理了 再入国許可（含 みなし再入国）吗？"
    type: yes_no_unknown
result_logic:
  green: "q1 == no AND q2 == no AND q3 == yes"
  yellow: "q1 == unknown OR q2 == unknown OR q3 == unknown"
  red: "q1 == yes OR q2 == yes OR q3 == no"
result_actions:
  red:
    - "单次 / 累计超期 → 续签 / 永住 連続性 受影响，建议咨询行政書士"
    - "未取得通常再入国许可，且出国时也没有使用或满足みなし再入国条件 → 在留資格 可能失效（含 永住）"
  yellow:
    - "向 出入国在留管理庁 申请「出入国記録 開示請求」获得完整记录"
    - "对照档案保留的 出入国 ED カード 副本"
---

# 出入国记录 / 连续居住

## 这是什么
出入国記録 是 出入国在留管理庁 保存的本人入出境完整履历。再入国許可（包括 みなし再入国 1 年 / 正規 最長 5 年）= 在留資格 持有人出国期间不丧失资格的法定手续。永住者 同适用。

## 为什么重要
- 在留期間更新：経営・管理 持有人长期海外（如管理海外子公司、长期回国谈业务）= 经营管理活动实体在日本不足，续签 时被怀疑「事業実体不稳」，可能给较短期或不予更新。
- 永住申請：「引き続き 10 年」要件 = 看出入国記録 而非物理时长。单次出国 90 日以上 / 累计 100 日以上 在审查中是消极信号（待书士确认具体口径）。
- 全签证：未取得通常再入国许可，且出国时也没有使用或满足みなし再入国条件时，在留資格可能失效，需按出国手续、期限和入境记录确认。

## 判断标准
- **绿**：1 年内单次出国 ≤ 90 日 + 累计 ≤ 100 日 + 全程办了再入国許可
- **黄**：天数 / 手续不确定
- **红**：单次或累计超 + 或未办手续

## 处理建议
### 绿
档案归档每次出入国时点 + ED カード 控（写）/ みなし选项确认。续签 / 永住 前自查累计天数。経営・管理 应同时保留事務所 在日本运营的客观证据（事務所賃貸契約、社員出勤、銀行流水）以对冲出国天数。

### 黄
1. 向 [出入国在留管理庁](https://www.moj.go.jp/isa/) 申请「出入国記録に係る開示請求」（书面，回答 2-4 周）。
2. 对照档案 出入国 履历 + 護照入出国印章。

### 红
1. 单次 / 累计超期 → 永住「引き続き」可能中断，需重新累计。续签 也可能给较短期。経営・管理 还会被审查在出国期间事業是否实际运营。
2. 未取得通常再入国许可，且出国时也没有使用或满足みなし再入国条件 → 在留資格可能失效，需按出国手续、期限和入境记录确认。
3. 严重情况建议带材料向行政书士确认。

## 相关材料
- 出入国記録 開示請求 結果
- 護照（入出国印章页）
- 再入国許可書 控（写）/ ED カード 副本
- 在留カード（含「再入国許可」栏，如有正規许可）
- 出国期间事業运营客观证据（取締役会議事録、銀行取引、メール記録）

## 价值 expiry 说明
expiry_days = 30。出入国 状况按出行随时变化，30 日内须复检；永住申請 关键节点前重新核对完整记录。
