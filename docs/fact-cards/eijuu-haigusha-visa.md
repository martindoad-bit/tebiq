---
fact_id: eijuu-haigusha-visa
title: 永住者の配偶者等ビザの申請要件と注意点
state: ai_verified
risk_level: high
confidence: medium
source_quality: official
controlled_alpha_eligible: false
last_verified_at: "2026-05-07"
reviewer: FACT-OPS (Batch 8)
sprint: "0.6"
ai_pipeline: WebFetch → FACT-OPS review
official_sources:
  - url: https://www.moj.go.jp/isa/applications/status/spouseorchildofpermanentresident.html
    label: 出入国在留管理庁 — 永住者の配偶者等（在留資格）
    accessed: "2026-05-07"
applies_to:
  - 永住者または特別永住者と婚姻している外国人配偶者
  - 永住者または特別永住者の子として日本で生まれ引き続き在留する者
  - 永住者の配偶者等ビザの更新・変更を検討する者
direct_fact_fields:
  - 対象者：「永住者または特別永住者の配偶者、または日本で生まれ引き続き在留している永住者・特別永住者の子」
  - 在留期間の種類：「5年、3年、1年または6か月」の4種類
  - 主な申請必要書類：婚姻証明書（本国発行）、財力証明（課税証明・銀行残高）、永住者配偶者の身元保証書、世帯全員の住民票、写真・質問書
ai_inferred_fields:
  - 就労制限なし（日本人の配偶者等と同様、活動内容の制限なし）
  - 更新時に婚姻の実態（同居・生活実態）の確認がある
  - 6か月以上「配偶者として活動していない」場合は在留資格取消対象（入管法22条の4）
  - 日本人配偶者等との比較：永住者スポンサーと日本人スポンサーの違いは手続きに影響しないが、永住者自身が在留資格を持つ外国人であるため書類関係が異なる
  - 永住者配偶者等ビザ → 永住申請の基礎要件（3年以上かつ1年以上継続在留で永住申請資格）
needs_review_flags:
  - no_work_restriction_source: 就労制限なしは法律上の解釈に基づくai推定。入管法別表第2「永住者の配偶者等」の活動範囲条文確認要。
  - marital_substance_review: 更新時の婚姻実態審査基準の公式ガイドライン確認要（nihonjin-haigusha-visaカードと同様の論点）。
  - eijuu_vs_nihonjin_spouse_diff: 永住者配偶者等と日本人配偶者等の審査実務上の差異の確認要（スポンサーが外国籍永住者の場合の実務的違い）。
---

## current_date_logic

本カードの内容は {{TODAY_ISO}} 時点での規則に基づく。本カードは `nihonjin-haigusha-visa` カードと類似するが、スポンサーが「日本人」ではなく「永住者（外国籍）」である点が異なる。離婚後の問題は `spouse-divorce-separation` カード参照。

## current_effective_fact

「永住者の配偶者等」は永住者（または特別永住者）との実態ある婚姻・血縁関係に基づく身分系在留資格。

**主要事実（{{TODAY_ISO}} 現在）：**
- 対象：永住者・特別永住者の配偶者、および日本生まれで継続在留中の永住者・特別永住者の子
- 在留期間：5年・3年・1年・6か月（審査結果による）
- **就労制限なし**（活動内容の制限がない）【ai推定・`no_work_restriction_source`確認要】

**日本人の配偶者等ビザとの主な違い：**
| 項目 | 永住者の配偶者等 | 日本人の配偶者等 |
|------|---------------|--------------|
| スポンサー | 永住者・特別永住者 | 日本国籍者 |
| 在留期間 | 5年・3年・1年・6か月 | 5年・3年・1年・6か月 |
| 就労制限 | なし（推定） | なし |
| スポンサー変動 | スポンサー死亡・離婚でリスクあり | 離婚・死別でリスクあり |
| 申請書類 | 財力証明・身元保証書等が必要 | 日本人配偶者の証明等が必要 |

## exceptions_or_transition

- **スポンサー（永住者）の死亡**: 死亡した場合、婚姻ビザの根拠が消滅するため在留資格変更が必要（定住者への変更等）
- **離婚後**: 永住者との離婚後も日本に在留するには在留資格変更が必要。日本人との離婚と同様のリスク（`spouse-divorce-separation` 参照）
- **永住者配偶者等→永住申請**: 永住者配偶者等で3年以上（かつ1年以上継続）在留すれば永住申請の基礎要件を満たす可能性あり（他の永住条件も満たす必要あり）
- **日本で生まれた子の対象**: 「日本で生まれ引き続き在留している永住者の子」のみが対象。日本生まれでない子や一時的に海外在留後に再入国した子は要件を別途確認
- 身元保証人は永住者本人が担う（財力・身元保証の書類が必要）

## common_user_phrases

- 永住者の夫（妻）と結婚しました、どのビザを取ればいいですか
- 永住者の配偶者ビザで働けますか
- 永住者の旦那と別居しています、ビザは大丈夫ですか
- 永住者の配偶者ビザと日本人配偶者ビザの違いは何ですか
- 永住者の子供として日本で生まれましたが、在留資格はどうなりますか
- 永住者の配偶者等ビザから永住申請はいつできますか
- 永住者の夫が亡くなりました。ビザはどうなりますか

技術キーワード（マッチャ用）：

- 永住者 配偶者 / 永住者の配偶者等 / 永住 結婚 ビザ
- 永住者 妻 ビザ / 永住者 夫 ビザ / 永住者と結婚
- 永住者 配偶者 就労 / 働ける 永住者配偶者
- 永住者 配偶者 更新 / 婚姻実態 永住者
- 永住者の子 日本生まれ / 永住者 子供 在留
- 永住者 配偶者 離婚 / 永住 スポンサー死亡

## must_say

- 永住者の配偶者等ビザは就労制限なし（活動内容の制限がない）【ai推定確認要】
- 更新時に婚姻の実態が確認される。別居・連絡断絶は更新リスクあり
- 永住者スポンサーが死亡・離婚した場合は在留資格変更が必要
- 「永住者の子」の対象は日本生まれで継続在留している子に限定される

## must_not_say

- 「日本人の配偶者等ビザと全く同じ」（スポンサーが外国籍永住者である点で書類・実務が異なる）
- 「永住者の夫がいれば在留は自動的に保証される」（更新審査・実態確認がある）
- 「永住者のスポンサーが死亡しても大丈夫」（根拠が消滅するため変更が必要）

## qa_cases

**Q1: 永住者の夫と結婚しました。私はフルタイムで働きたいのですが、永住者の配偶者等ビザで就労できますか？**
A: 永住者の配偶者等は就労制限がない身分系在留資格のため、フルタイム就労が可能です（ai推定・`no_work_restriction_source`確認要）。活動内容に制限がなく、日本人の配偶者等と同様に扱われます。

**Q2: 永住者の配偶者等ビザと日本人の配偶者等ビザは何が違いますか？**
A: 在留期間や就労の扱いは基本的に同様ですが、スポンサーが「日本国籍者」か「外国籍の永住者」かという違いがあります。申請書類（スポンサーの在留カード・身元保証書の内容等）に違いがあります。婚姻実態の審査要件は同様と考えられます。

**Q3: 永住者の夫が突然亡くなりました。日本在留を続けられますか？**
A: 婚姻に基づく在留資格の根拠が消滅します。在留資格の変更申請が必要です。日本に子供がいる場合や長期在留の実態がある場合、定住者等への変更を申請する際に考慮事由となります。地方出入国在留管理局への早急な相談を推奨します。

## injection_format

injection_certain_block: |
  【永住者の配偶者等ビザ ファクト / {{TODAY_ISO}} 確認済み】
  ・対象：永住者・特別永住者の配偶者、および日本生まれで継続在留中の永住者の子
  ・在留期間：5年・3年・1年・6か月（審査で決定）
  ・就労制限なし（活動制限のない身分系在留資格）【ai推定確認要】
  ・更新時に婚姻実態（同居・生活状況）の確認あり
  ・離婚・スポンサー死亡時は在留資格変更が必要（詳細は spouse-divorce-separation 参照）
  ・主な申請書類：婚姻証明・財力証明・身元保証書・住民票等

injection_needs_review_addendum: |
  【要確認情報（AI推定・非注入）】
  ・就労制限なしの法令根拠（入管法別表第2確認）（no_work_restriction_source）
  ・婚姻実態審査の具体的基準（marital_substance_review）
  ・日本人配偶者等との審査実務上の差異（eijuu_vs_nihonjin_spouse_diff）

## changelog

| 日付 | 担当 | 変更内容 | state_before | state_after | タグ |
|------|------|----------|--------------|-------------|------|
| 2026-05-07 | FACT-OPS (Batch 8) | 新規作成。永住者の配偶者等の対象・在留期間は公式ソース確認済み。就労制限なし・更新実態審査はai_inferred。nihonjin-haigusha-visaとの棲み分け明記（スポンサーが永住者である点を中心に）。 | — | ai_verified | new |
