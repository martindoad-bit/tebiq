...
ai_pipeline:
  ...
  extractor_model: deepseek-v4-pro (manual-review via Claude Opus 4.7)
  source_count: 1
official_sources:
  - id: moj-isa-10-00237
...
needs_human_review_reasons:
  - "5項目目（事業計画書 専門家確認）が当該公式ページの抜粋に明示されていない。改正ガイドライン PDF で要確認。"
  - "日本語能力 B2 相当の認定方式（JLPT N2 / BJT 400+ / 学歴等）の網羅性は AI 推論により補完。要 PDF 確認。"
  - "事業所要件の面積基準は『一律答え難し』とのみあり、運用具体例は要追加調査。"
---
...
### 6. 事業計画書の専門家確認【needs_human_review】
...
When this card matches and `state: human_approved`, the matcher
should inject the following block into the system prompt **before**
the user message:

```text
【今日の有効な事実 — 経営・管理ビザ 2025年10月16日改正】

以下は出入国在留管理庁の公式公表に基づく現行基準である。
今日の日付は {{TODAY_ISO}} であり、施行日 (2025-10-16) を経過している。

新規申請・在留資格変更の場合、以下の要件を同時に満たすことが必要：
1. 資本金または出資の総額が 3,000万円以上
2. 常勤職員 1名以上の雇用
   （日本人、特別永住者、別表第二在留資格保持者に限る）
3. 申請者が経営・管理に関する博士/修士/専門職学位を有する、
   または事業経営・管理について 3年以上の経験を有する
4. 申請者または常勤職員のいずれかが日本語 N2 相当以上の能力を有する
5. 事業所は原則として自宅と兼用不可

旧基準（500万円基準）は、2025-10-15 までに申請を受付・審査継続中の
案件にのみ適用される。既存の経営・管理ビザ保持者の更新については、
施行日から3年（〜2028-10-15）の経過措置あり。

公式参照: https://www.moj.go.jp/isa/applications/resources/10_00237.html

回答時の禁止事項：
- 「通常 500万円以上で申請可」と現行新規申請者向けに言及しない
- 「2025-10-15 までに急いで提出」と新規申請者を誘導しない
- 5要件のいずれかを省略しない
```
