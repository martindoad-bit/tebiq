...
- 无来源、无 QA、无 DOMAIN 队列的大规模知识库扩写

这些不是永久冻结，是**等数据**。

说明：source-traceable 的 FACT / legal-source / Materials checklist 生产不属于
"盲目知识库扩写"。它可以继续推进，但必须保留来源、审核队列、runtime promotion
边界，不能直接把未审卡片注入用户端确定性回答。
